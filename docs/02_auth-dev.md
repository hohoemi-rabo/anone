...
- [ ] `hooks/use-auth.ts` 作成（signIn, signUp, signOut, session, isLoading を提供）
- [ ] `AuthProvider` コンテキスト作成（セッション状態をアプリ全体に配信）
- [ ] Supabase `onAuthStateChange` でセッション変更を監視

### 認証画面

- [ ] `app/sign-in.tsx` 作成（Email/Password ログインフォーム）
- [ ] `app/sign-up.tsx` 作成（開発用アカウント作成画面）

### ルート保護

- [ ] `app/_layout.tsx` を `Stack.Protected` パターンに変更
- [ ] 認証済み → `(tabs)` グループへ、未認証 → `sign-in` へ
- [ ] ディープリンクからのアクセスも認証チェックされることを確認

### users テーブル連携

- [ ] サインアップ時に `users` テーブルにレコード作成（Supabase Auth trigger または手動 insert）
- [ ] セッションから `user.id` を取得し各画面で利用できるようにする
